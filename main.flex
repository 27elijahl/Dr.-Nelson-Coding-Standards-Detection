...
5.  Block comments must not be indented or consistently indented once.
...
13. There should be a line of whitespace before the line defining a loop
    or conditional and after the closing brace, if the indentation is the same, excepting else statements following if statements.
14. For loops should have a space after "for", should have spaces after
    the semicolons (if succeeded by a statement), and should have spaces
    around "=", "<", etc.
15. if (...) ...;
    else ... is invalid.
...
17. A non-void function must have a return statement as the last
    executable line and no other return statements. A void function must
    also have a return statement as the last line if the language is
    detected to be C (in the absence of a class or interface 
    definition).
...
4. Semantics of a for loop ("for loops should not simulate while loops")
